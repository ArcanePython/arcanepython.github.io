...
export class Canvas3dTextureScene2
implements scene.SceneInterface
{
    ...
    textureOn3DCube: WebGLTexture |undefined;
    
    widthTextureToRenderOn = 512;
    heighttextureToRenderOn = 512;
    textureToRenderOn: WebGLTexture |undefined;
 ...
 fragmentShaderSource = `#version 300 es
    ...
    void main() {
      outColor = vec4(texture(u_texture, v_texcoord).rrr, 1) * u_colorMult*2.0;
    }
    `;
...
gl.bufferData(gl.ARRAY_BUFFER, positions, gl.STATIC_DRAW);
...

  {
    ...
    gl.texImage2D(gl.TEXTURE_2D, level, internalFormat,
                  this.widthTextureToRenderOn, this.heighttextureToRenderOn, border,
                  format, type, data);
...
//  requestAnimationFrame(drawScene);
  ...
  drawCube(gl: WebGL2RenderingContext, program: WebGLProgram, aspect:number,cam: camhandler.Camera|null) {
   ...
   if (cam==null)
   {
        ...
        matrix = m4.axisRotate(matrix,[1,0,0], this.modelXRotationRadians * 1.0);
        matrix = m4.axisRotate(matrix,[0,1,0], this.modelYRotationRadians * 1.0);
        ...
        var c = 1;
        gl.uniform4fv(this.colorMultLocation!, [c * .5 + .5, 1, 0, 1]);
   ...
   } else
    {
      ...
      for (let x = -1; x <= 1; ++x) 
      {
          ...
          if(cam!=null) matrix = m4.translate(matrix,[-x*16.0,0,0]);
