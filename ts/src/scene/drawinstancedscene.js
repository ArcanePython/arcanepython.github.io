...
`;
    ...
    initScene(gl, cap, cam, dictpar, sceneReadyCallback) {
        ...
        // update the matrices
        for (var ii = 0; ii < this.numInstances; ii++) {
            var mat = this.matrices[ii];
            // uneven instances are Y-direction, even instances are X-direction
            if ((ii % 2) == 1) {
                twgl_js_1.m4.axisRotation([0, 1, 0], -Math.PI / 2.0, mat);
                twgl_js_1.m4.translate(mat, [tr[0] + 2 * Math.floor(ii / 2) * interspace, tr[1], tr[2]], mat);
            }
            else {
                twgl_js_1.m4.axisRotation([0, 1, 0], 0.0, mat);
                twgl_js_1.m4.translate(mat, [tr[0] - (2 * Math.floor(ii / 2)) * interspace, tr[1], tr[2]], mat);
            }
        }
        // update the colors
