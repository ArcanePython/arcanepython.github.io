...
export class mat4tools
{

   quatFromRotationMatrix(m: m4.Mat4, dst: [number,number,number,number]) {
  ...
  lengthnum4(v: [number,number,number]) {
    return Math.sqrt(v[0] * v[0] + v[1] * v[1] + v[2] * v[2]);
  }

  lengthfa(v: Float32Array) {
    return Math.sqrt(v[0] * v[0] + v[1] * v[1] + v[2] * v[2]);
  }

  determinate(m: m4.Mat4) {
    var m00 = m[0 * 4 + 0];
    var m01 = m[0 * 4 + 1];
    var m02 = m[0 * 4 + 2];
    var m03 = m[0 * 4 + 3];
  ...
  decompose(mat: m4.Mat4, translation: m4.Mat4, quaternion:  [number,number,number,number], scale: [number,number,number]) {
     ...
     */
    compose(translation: number[], quaternion: number[], scale:number[], dst:m4.Mat4) {
      //dst = dst || new m4.Mat4();
